...
} from "store/slices/materialScheduleSlice";
...
const AddMaterialModal = ({
  ...
  const _submit = async (
    ...
    const { e, response } = await postForm(
      "patch",
      `procurements/material-schedule/add-material`,
      payload
    );
    if (response) {
  ...
  const _edit = async (
    ...
    const { e, response } = await postForm(
      "patch",
      `procurements/material-schedule/update-material`,
      payload
    );
    if (response) {
    ...
    >
      ...
      >
        ...
        <SelectDate
          className=""
          error={
            ((touched.purchaseDate && errors?.purchaseDate) || "") as string
          }
          placeholder="Select date"
          wrapperClassName=" !border-ashShade-4 "
          minDate={new Date(0)}
          initialValue={
            value?.purchaseDate ? new Date(value.purchaseDate) : new Date()
          }
          value={date}
