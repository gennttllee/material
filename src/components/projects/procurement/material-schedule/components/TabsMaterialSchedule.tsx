...
const TabsMaterialSchedule: FC<ScheduleType> = ({
  ...
  const [editing, setEditing] = useState<MaterialScheduleRecord | undefined>(
    undefined
  );
  ...
  const handleEditing =
    (
      val: MaterialScheduleRecord,
      setter: (x: MaterialScheduleRecord) => void
    ) =>
    () => {
      setter(val);
    };
