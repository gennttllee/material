...
export const Gantt: React.FunctionComponent<GanttProps> = ({
  ...
  const [todayFocusHasFired, setTodayFocus] = useState(false);

  // task change events
  useEffect(() => {
    let filteredTasks: CustomTask[];
    if (onExpanderClick) {
      ...
      for (const el of [targetElement]) {
        ...
        for (const [key, value] of Object.entries(el))
          if (key.includes('reactProps')) {
            // @ts-ignore
            setScrollX(value.x < 400 ? 0 : value.x - 400);
            // @ts-ignore
            setScrollY(value.y);
          }
      }
  ...
  useEffect(() => {
    if (!todayFocusHasFired) {
      focusOnToday();
      setTodayFocus(true);
    }
  }, [tasks, todayFocusHasFired]);

  useEffect(() => {
    if (parentScroll) {
      setScrollX(parentScroll.x);
      // @ts-ignore
      setScrollY(parentScroll.y);
    }
  }, [parentScroll]);
  ...
  }, [wrapperRef.current, scrollY, scrollX, ganttHeight, svgWidth, rtl, ganttFullHeight]);
  ...
  const barProps: TaskGanttContentProps = {
    ...
    onClick,
    onDelete,
    onExpanderClick: (t: CustomTask) => {
      if (onExpanderClick) onExpanderClick(t);
    }
  };

  const tableProps: TaskListProps = {
    rowHeight: (rowHeight * barFill) / 100,
  ...
  return (
    <div className="h-[80%] w-full 2xl:h-[90%] overflow-hidden relative z-10">
      <div
        className={styles.wrapper + ' h-full w-full bg-white overflow-scroll'}
