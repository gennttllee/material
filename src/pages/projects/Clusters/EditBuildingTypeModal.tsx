...
const EditBuildingTypeModal = ({ closer, details }: EditBuildingTypeModalProps) => {
  ...
  const handleEdit = async () => {
    if (!loading) {
      setLoading(true);
      let { e, response } = await postForm('patch', `clusters/update-type`, {
        ...data[0],
        typeId: details._id
      });
      if (e) {
        displayError('Cannot Edit Cluster Building type');
      } else {
        displaySuccess('Cluster Building Type Modified successfully');
        dispatch(updateCluster(response.data.data));
        closer()
      }
      setLoading(false);
      ...
      <div className=" w-full h-full flex items-start justify-center bg-black bg-opacity-40 overflow-y-auto pb-10 ">
        <div
          ...
          <div className="flex items-center justify-between">
            <span className=" text-bblack-0 text-2xl font-semibold">Edit Building Type</span>
          ...
          <div className="w-full flex items-center justify-between">
            <span
              ...
              className=" invisible  hover:underline rounded-md hover:cursor-pointer items-center flex ">
